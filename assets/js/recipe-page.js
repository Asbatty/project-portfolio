...
(function () {
  ...
  if (versions.length > 1) {
    ...
    switcherSel = document.createElement('select');
    switcherSel.className = 'version-select';
    switcherSel.setAttribute('aria-label', 'Recipe version');
    versions.forEach((v, i) => {
      const opt = document.createElement('option');
      opt.value = i;
      opt.textContent = 'v' + v.version + (v.date ? ' — ' + v.date : '') + (i === versions.length - 1 ? ' (current)' : '');
      switcherSel.appendChild(opt);
    });
    switcherSel.value = String(currentIdx);
    switcherSel.addEventListener('change', () => { currentIdx = Number(switcherSel.value); renderVersion(); });
    const note = document.createElement('span');
    note.className = 'version-note';
    note.id = 'rp-version-note';
    wrap.appendChild(label);
    wrap.appendChild(switcherSel);
    wrap.appendChild(note);
    content.appendChild(wrap);
  }

  const versionHost = document.createElement('div');
  versionHost.id = 'rp-version-host';
  ...
  function renderVersion() {
    ...
    const note = $('rp-version-note');
    if (note) {
      note.textContent = (currentIdx !== versions.length - 1)
        ? 'Older build — highlights show what changed from v' + (prev ? prev.version : '?')
        : (prev ? 'Highlights show what changed from v' + prev.version : '');
      if (!prev && note) note.textContent = '';
    }
